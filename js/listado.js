...
// REFACTOR de obtenerCiudades() --> Obtiene todas las ciudades de los restaurantes sin repetidos
Listado.prototype.obtenerCiudades = function() {
    ...
    return this.eliminarRepetidos(ciudades).sort();
}
...
// REFACTOR de obtenerHorarios() --> Obtiene todos los horarios de los restaurantes (sin repetidos). Está funcionalidad es un poco más compleja ya que un restaurante
...
Listado.prototype.obtenerHorarios = function() {
    ...
    // Retornamos un arreglo con elementos unicos y ordenados
    return this.eliminarRepetidos(horarios).sort();
}

//Guia 2 - Paso 4 --> Esta función se utiliza en obtenerCiudades, obtenerRubros y obtenerHorarios para evitar repetir código
Listado.prototype.eliminarRepetidos = (listaElementos)=> {
    return listaElementos.filter((elem, index, self) => index === self.indexOf(elem)
    );
}
